...
def main():
    ...
    rpnl_l1: List[float] = []
    diffs: List[float] = []
    trades_dqn: List[float] = []
    trades_l1: List[float] = []
    for i, fname in enumerate(common, start=1):
        ...
        diffs.append(di)
        rpnl_dqn.append(dqn_realised)
        rpnl_l1.append(l1_realised)
        trades_dqn.append(len(dqn_df))
        trades_l1.append(len(l1_df))
    ...
    rpnl_dqn_np = np.asarray(rpnl_dqn,dtype=float)
    rpnl_l1_np = np.asarray(rpnl_l1,dtype=float)
    n = int(diffs_np.size)
    mean_diff, sd_diff = mean_sd(diffs)
    mean_rpnl_dqn, sd_rpnl_dqn = mean_sd(rpnl_dqn)
    mean_rpnl_l1, sd_rpnl_l1 = mean_sd(rpnl_l1)
    mean_trades_dqn,sd_trades_dqn=mean_sd(trades_dqn)
    mean_trades_l1,sd_trades_l1=mean_sd(trades_l1)
    print("\n# Summary")
    print(f"(DQN) realised pnl - mean: {mean_rpnl_dqn}, sd:{sd_rpnl_dqn},   trades - mean:{mean_trades_dqn}")
    print(f"(L1) realised pnl - mean: {mean_rpnl_l1}, sd:{sd_rpnl_l1},   trades - mean:{mean_trades_l1}")
    print("\n#(paired test)")
    print(f"n       = {n}")
    print(f"mean di = {mean_diff:.6f}")
    print(f"sd di   = {sd_diff:.6f}")
    ...
    if n >= 2 and sd_diff > 0:
        ...
        # Wilcoxon (one-sided: median > 0), zeros dropped automatically with zero_method="wilcox"
        try:
            w_stat, wilcoxon_p = st.wilcoxon(diffs_np, alternative="greater", zero_method="wilcox")
            wilcoxon_p = float(wilcoxon_p)
        except Exception:
            wilcoxon_p = float("nan")

        print(f"t-test  : t = {t_stat:.6f}, df = {dfree}, p_value = {p_two:.6g}")
        print(f"95% CI  : [{ci_lo:.6f}, {ci_hi:.6f}]")
        print(f"Wilcoxon: p_value = {wilcoxon_p:.6g}")
    else:
        print("t-test  : NA (need at least n>=2 and nonzero sd).")
        print("95% CI  : NA")
        # Still try Wilcoxon if we have SciPy and at least one diff
        if n >= 1:
            try:
                w_stat, wilcoxon_p = st.wilcoxon(diffs_np, alternative="greater", zero_method="wilcox")
                print(f"Wilcoxon: p_value= {float(wilcoxon_p):.6g}")
            except Exception:
                print("Wilcoxon: NA")
